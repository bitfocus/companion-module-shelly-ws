class ShellyRelayMaster {
  ...
  getVariableValues() {
    ...
    // Input states
    for (let i = 0; i < this.inputCount; i++) {
      variableValues[`input_${i + 1}_state`] =
        this.inputStates[i] != undefined ? this.inputStates[i] : false;
    }
  ...
  getFeedbackDefinitions() {
    ...
    const relayOptions = Array.from(
      { length: this.relayCount },
      (_, index) => ({
        id: index,
        label: `Relay ${index + 1}`,
      })
    ...
    return {
      switchRelay: {
        ...
        callback: async (action, context) => {
          this.switchRelay(
            action.options.selectedRelay,
            action.options.selectedRelayState
          );
        },
      },
      toggleRelay: {
        ...
        callback: async (action, context) => {
          ...
          var targetState =
            this.relayStates[action.options.selectedRelay] == true ? 1 : 0;
...
class ShellyMasterCover {
  ...
  getActionDefinitions() {
    const coverOptions = Array.from(
      { length: this.coverCount },
      (_, index) => ({
        id: index,
        label: `Cover ${index + 1}`,
      })
    ...
    return {
      ...
      goToPos: {
        ...
        callback: async (action, context) => {
          this.goToPosition(
            action.options.selectedCover,
            action.options.targetPosition
          );
        },
      },
    };
      ...
      coverStateBool: {
        ...
        callback: (feedback, context) => {
          switch (feedback.options.selectedCoverState) {
            case 0:
              return this.coverStates[feedback.options.selectedCover] ==
                "opening"
                ? true
                : false;
              break;
